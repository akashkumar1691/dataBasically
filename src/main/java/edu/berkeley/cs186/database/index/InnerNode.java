...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) throws BPlusTreeException {
        // TODONE(proj2): implement
        BPlusNode babyNode = getChild(numLessThanEqual(key, keys));
        Optional<Pair<DataBox,Long>> opcion = babyNode.put(key, rid);
        sync();
        if(opcion.isPresent()){
            DataBox babySplitkey = opcion.get().getFirst();
            long right_node_page_num = opcion.get().getSecond();
            int i = numLessThanEqual(babySplitkey, keys);
            keys.add(i, babySplitkey);
            children.add(i+1, right_node_page_num);
            opcion = Optional.empty();
            int o = metadata.getOrder();
            i = o+1;
            if(keys.size() > 2* o){
                List<DataBox> rightKeys = new ArrayList<>();
                List<Long> rightChildren = new ArrayList<>();
                rightChildren.add(children.remove(i));

                while(i < keys.size()){
                    rightKeys.add(keys.remove(i));
                    rightChildren.add(children.remove(i));
                }
                DataBox splitKey = keys.remove(o);
                BPlusNode right = new InnerNode(metadata, bufferManager, rightKeys, rightChildren, treeContext);
                opcion = Optional.of(new Pair<>( splitKey, right.getPage().getPageNum()));
            }
        }
        sync();
        return opcion;
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODONE(proj2): implement
        Optional<Pair<DataBox,Long>> opcion =  Optional.empty();
        int capacity = 2*metadata.getOrder();
        int splitFactor = capacity +1;
        Optional<Pair<DataBox, Long>> childSplit;
        while(data.hasNext() && keys.size() <= capacity){
            childSplit = (getChild(children.size()-1)).bulkLoad(data, fillFactor);
            if(childSplit.isPresent()){
                keys.add(childSplit.get().getFirst());
                children.add(childSplit.get().getSecond());
            }
        }

        if(keys.size() == fillFactor){
            int o = metadata.getOrder();
            int i = o+1;
            List<DataBox> rightKeys = new ArrayList<>();
            List<Long> rightChildren =  new ArrayList<>();
            ...
            opcion = Optional.of(new Pair<>(splitKey, rightNode.getPage().getPageNum()));
        }
        sync();
        return opcion;
    }
