...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException {
        typecheck(key);
        // TODONE(proj2): implement
        Optional<Pair<DataBox,Long>> opcion = root.put(key,rid);
        if(opcion.isPresent()){
            List<DataBox> rootKeys = new ArrayList<>();
            List<Long> rootChildren = new ArrayList<>();
            rootKeys.add(opcion.get().getFirst());
            rootChildren.add(root.getPage().getPageNum());
            rootChildren.add(opcion.get().getSecond());
            updateRoot(new InnerNode(metadata, bufferManager, rootKeys, rootChildren, lockContext));
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) throws BPlusTreeException {
        // TODONE(proj2): implement
        if(scanAll().hasNext()){
            throw new BPlusTreeException("stop! can't bulkload non-empty tree");
        }
        Optional<Pair<DataBox, Long>> opcion;
        while (data.hasNext()) {
            opcion =root.bulkLoad(data, fillFactor);
            if(opcion.isPresent()){
                List<DataBox> rootKeys = new ArrayList<>();
                List<Long> rootChildren = new ArrayList<>();
                rootKeys.add(opcion.get().getFirst());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        // TODONE(proj2): Add whatever fields and constructors you want here.
        ...
        private Optional<LeafNode> nxtFullNode(LeafNode leaf){
            Optional<LeafNode> rtn = leaf.getRightSibling();
            if(rtn.isPresent()){
                while (rtn.isPresent() && rtn.get().getKeys().size() == 0) {
                    rtn = rtn.get().getRightSibling();
                }
                return rtn;
            } else{
                return rtn;
        ...
        public boolean hasNext() {
            ...
            if(currIter.hasNext()){
                ...
                Optional<LeafNode> next = nxtFullNode(currNode);
                if(next.isPresent()){
                    currNode = next.get();
                    currIter = currNode.scanAll();
                }
