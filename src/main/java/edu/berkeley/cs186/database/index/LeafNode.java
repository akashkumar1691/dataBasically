...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) throws BPlusTreeException {
        if(getKey(key).isPresent()){
            ...
            if(keys.size() > 2* metadata.getOrder() ){
                ...
                while(o < keys.size()){
                    rightKeys.add(keys.remove(o));
                    rightRids.add(rids.remove(o));
                }
                ...
                opcion = Optional.of(new Pair<DataBox, Long>(rightNode.keys.get(0), rightNode.getPage().getPageNum()));
            }
            sync();
            return opcion;
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int fillCapacity = (int) Math.ceil((2*metadata.getOrder())*fillFactor);
        int splitFactor = fillCapacity + 1;
        Optional<Pair<DataBox, Long>> opcion = Optional.empty();
        while(data.hasNext() && keys.size() <= fillCapacity){
            Pair<DataBox, RecordId> newdata = data.next();
            DataBox newkey = newdata.getFirst();
            if(getKey(newkey).isPresent()){
                throw new BPlusTreeException(":(");
            }
            RecordId newrid = newdata.getSecond();

            int i = InnerNode.numLessThanEqual(newkey, keys);
            keys.add(i, newkey);
            rids.add(i, newrid);
        }

        if(keys.size() == splitFactor){
            List<DataBox> rightKeys = new ArrayList<>();
            List<RecordId> rightRids = new ArrayList<>();

            rightKeys.add(keys.remove(keys.size()-1));
            rightRids.add(rids.remove(rids.size()-1));
            LeafNode rightNeighbor = new LeafNode(metadata, bufferManager,rightKeys, rightRids,rightSibling, treeContext);
            rightSibling = Optional.of(rightNeighbor.getPage().getPageNum());
            opcion = Optional.of(new Pair<>(rightKeys.get(0),rightSibling.get()));
        } else if(keys.size()> splitFactor){
            throw new BPlusTreeException("weeeeeird");
        }
        sync();
        return opcion;
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long rightneighbor = buf.getLong();
        Optional<Long> rightSibling;
        if (rightneighbor == -1L) {
            rightSibling = Optional.empty();
        } else {
            rightSibling = Optional.of(rightneighbor);
        }

        List<DataBox> keys = new ArrayList<>();
        List<RecordId> rids = new ArrayList<>();
        int records = buf.getInt();
