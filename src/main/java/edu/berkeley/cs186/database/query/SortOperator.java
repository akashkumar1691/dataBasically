...
public class SortOperator {
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        if(runs.isEmpty()) {
            ...
            List<Run> rtn = new ArrayList<>();
            for(int i = 0; i < runs.size(); i+= maxRuns){
                rtn.add(mergeSortedRuns(runs.subList(i, Math.min(i+maxRuns, runs.size()))));
            }
            return rtn;
        }
    }
    ...
    public String sort() {
        ...
        BacktrackingIterator<Page> iter =  transaction.getPageIterator(tableName);
        while (iter.hasNext()) {
            Run pass0 = sortRun(createRunFromIterator(transaction.getBlockIterator(tableName,iter,numBuffers)));
            sort.add(pass0);
        }

        while(!(sort.size() <= 1)){
            mergeSortedRuns(sort);
        }
