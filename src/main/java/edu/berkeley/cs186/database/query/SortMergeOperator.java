    ...
    private class SortMergeIterator extends JoinIterator {
        ...
        */
        ...
        private SortMergeIterator() {
            ...
            SortOperator leftOp = new SortOperator(getTransaction(), getLeftTableName(), new LeftRecordComparator());
            SortOperator rightOp = new SortOperator(getTransaction(),getRightTableName(), new RightRecordComparator());
            leftIterator = getRecordIterator(leftOp.sort());
            rightIterator = getRecordIterator(rightOp.sort());
        ...
        private void fetchNextRecord(){
            if (this.leftRecord == null || this.rightRecord == null) { throw new NoSuchElementException("No new record to fetch"); }
            this.nextRecord = null;
            do {
                if (!marked) {
                    setMarked();
        ...
        private void setMarked(){
            ...
            while(leftJoinValue.compareTo(rightJoinValue) < 0){
                if(!leftIterator.hasNext()){
                    ...
                    leftRecord=leftIterator.next();
                    leftJoinValue = leftRecord.getValues().get(getLeftColumnIndex());
                }
            }
            while(leftJoinValue.compareTo(rightJoinValue) > 0){
                if(!rightIterator.hasNext()){
                    ...
                    rightRecord=rightIterator.next();
                    rightJoinValue= rightRecord.getValues().get(getRightColumnIndex());
                }
            }
            rightIterator.markPrev();
            marked = true;
