...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Set<String> startTableNames = new HashSet<>();
        startTableNames.add(startTableName);
        pass1Map.put(startTableNames, minCostSingleAccess(startTableName));
        for(String tableName: joinTableNames){
            Set<String> pass1Set = new HashSet<>();
            pass1Set.add(tableName);
            pass1Map.put(pass1Set, minCostSingleAccess(tableName));
        }
        ...
        for(prevMap = pass1Map; prevMap.size()>1; prevMap = minCostJoins(prevMap, pass1Map)){
            continue;
        }
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        for(int i: EligibleIndexColumns){
            curr = new IndexScanOperator(transaction, table,
                    selectColumnNames.get(i), selectOperators.get(i),
                    selectDataBoxes.get(i));
            currIO = curr.getIOCost();
            if(currIO < minIO){
                minOp = curr;
                minIO = currIO;
                except = i;
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for(Set<String> tableNames: prevMap.keySet()) {
            ...
            for(int i = 0; i < joinTableNames.size(); i++) {
                 ...
                 */
                if(tableNames.contains(left[0]) && !tableNames.contains(right[0])){
                    newTableNames.add(right[0]);
                    curr = minCostJoinType(prevMap.get(tableNames),pass1Map.get(newTableNames),
                    left[1], right[1]);
                }
                 /* Case 2. Set contains right table but not left, use pass1Map to
                 * fetch the right operator to access the leftTable.
                 */
                 else if(tableNames.contains(right[0]) && !tableNames.contains(left[0])){
                     newTableNames.add(left[0]);
                     curr = minCostJoinType(prevMap.get(tableNames), pass1Map.get(newTableNames),
                             right[1], left[1]);
                }
                ...
                newTableNames.addAll(tableNames);
                 if(!(map.containsKey(newTableNames))){
                     map.put(newTableNames, curr);
                 }else if(curr.getIOCost() < map.get(newTableNames).getIOCost()){
                     map.replace(newTableNames, curr);
                 }
            }
        }
        return map;
